import {toJSONPgnOptions} from "../typings/typings";

export class JSONUtils {
    /**
     * Removes: \n, and spaces from the PGN. In other words make the PGN string inline
     * */
    public trimPgn(Pgn: string) {
        var endPgn = ""
     ...
     * */
    public getTags(Pgn: string, options?: toJSONPgnOptions)  {
        var trimPgn = this.trimPgn(Pgn);
        var pgn = trimPgn.split(`"]`)
     ...
     * */
    public getMoves(Pgn: string, options: toJSONPgnOptions)  {
        var trimPgn = this.trimPgn(Pgn);
        var pgn = trimPgn.split(`"]`)
        // @ts-ignore
        pgn = pgn.pop().replace(" ","").split(/\d+\.(\.\.)?/g)
    ...
    public isPgn(Pgn: string) {
        ...
        try {
            ...
            Object.keys(tags).forEach(value => {
                if (needTags.includes(value)) counter += 1;
            })
